...
void CMNode::ext2cmCallback(const hellocm_msgs::Ext2CM::ConstPtr& msg) {
	std::stringstream ss;
...
void CMNode::cm2extFillMsg(hellocm_msgs::CM2Ext::Ptr& msg) {
...
int CMNode::userInit() {
	...
	{  // set up cm2ext publisher + job
		...
		job->setCycleTime(1000);
		job->setCycleOffset(0);
		job->registerCallback(&CMNode::cm2extFillMsg, this);
...
int CMNode::userTestrunStartAtBegin() {
	...
	{  // set up ext2cm subscriber + job
		...
		job->registerCallback(&CMNode::ext2cmCallback, this);
		scheduler_.addJob(job);
